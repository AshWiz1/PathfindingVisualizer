...
#buttons and fonts
startbtn = pygame.draw.rect(screen,[66, 245, 236],pygame.Rect(0,size[1],40,footer)) #dependency in mousebuttondown event
...
for i in range(row):
    for j in range(col):
        pygame.draw.rect(screen,[0,0,0],pygame.Rect(i*sizeblock[0],j*sizeblock[1],sizeblock[0]-2,sizeblock[1]-2),2)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.pos[0] >= 0 and event.pos[0] <= 40 and event.pos[1] >= size[1] and event.pos[1] < size[1]+footer :
                ...
                astar.row = row
                astar.col = col
                astar.sizeblock = sizeblock

                returncode = astar.solve()
                if returncode == -1:
                    f = False
            else:           
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.pos == mousedownpos:
            ...
            pygame.draw.rect(screen,[255,255,255],pygame.Rect(left,top,sizeblock[0],sizeblock[1]))
            pygame.display.update()
            if start == -1:
                flag+=1
                start = [i,j]
        ...
        if pygame.mouse.get_pressed()[0] and flag==2:
            try:
                ...
                grid[i][j] = cell(-1,-1)
                #print(i,j)
                pygame.draw.rect(screen,[255,0,255],pygame.Rect(left,top,sizeblock[0],sizeblock[1]))
                pygame.display.update()
               # print("as")
            except:
                pass
